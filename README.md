...
`skytreeLeague.excludeDatesWithOwnTeamActivity` が `true` の場合、管理画面の全募集区分から `skytreeLeague.ownTeamNames` がグランド取得チームまたは対戦申込チームになっている日付を検知し、その日のスカイツリーグ募集と都立公園空き枠をすべて通知対象外にします。デフォルトは有効で、自チーム名は `["ORDERMADE BASEBALL CLUB"]` です。大文字・小文字・空白・全角半角の違いは無視します。

「募集終了」に将来日程があり、自チームが対戦申込チームとして表示されている場合も自チーム関与日として除外します。
...
- 自チーム関与日: 自チームが募集側または申込側の日は、スカイツリーグ募集・都立公園空き枠ともに対象外
